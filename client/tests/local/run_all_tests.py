...
# -*- coding: utf-8 -*-
#
# Run all FAISSx local tests with progress display
#
# Copyright (C) 2025 Ran Aroussi

"""
Run all FAISSx local mode tests with progress display.

This script discovers and runs all test files in the local_tests directory,
...
def run_priority_tests():
    ...
    flat_module = import_module(
        os.path.join(script_dir, "test_index_flat.py"), "test_index_flat"
    )
    flat_suite = unittest.TestLoader().loadTestsFromTestCase(
        flat_module.TestIndexFlatL2
    )
    ...
    factory_module = import_module(
        os.path.join(script_dir, "test_factory.py"), "test_factory"
    )
    factory_suite = unittest.TestLoader().loadTestsFromTestCase(
        factory_module.TestIndexFactory
    )
    ...
    ivf_module = import_module(
        os.path.join(script_dir, "test_index_ivf_flat.py"), "test_index_ivf_flat"
    )
    ...
    idmap_module = import_module(
        os.path.join(script_dir, "test_index_idmap.py"), "test_index_idmap"
    )
    idmap_suite = unittest.TestLoader().loadTestsFromTestCase(
        idmap_module.TestIndexIDMap
    )
    idmap2_suite = unittest.TestLoader().loadTestsFromTestCase(
        idmap_module.TestIndexIDMap2
    )
    result = unittest.TextTestRunner(verbosity=1).run(
        unittest.TestSuite([idmap_suite, idmap2_suite])
    )
    print(
    ...
    persistence_module = import_module(
        os.path.join(script_dir, "test_persistence.py"), "test_persistence"
    )
    persistence_suite = unittest.TestLoader().loadTestsFromTestCase(
        persistence_module.TestIndexPersistence
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run FAISSx local tests")
    parser.add_argument(
        "--all", action="store_true", help="Run all tests via discovery"
    )
