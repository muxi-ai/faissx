...
if __name__ == "__main__":
    ...
    port = int(os.environ.get("FAISS_PORT", "45678"))
    bind_address = os.environ.get("FAISS_BIND_ADDRESS", "0.0.0.0")
    data_dir = os.environ.get("FAISS_DATA_DIR")
    enable_auth = os.environ.get("FAISS_ENABLE_AUTH", "").lower() in ["true", "1", "yes"]

    # Handle authentication - use either auth_keys or auth_file
    auth_keys = None
    auth_file = os.environ.get("FAISS_AUTH_FILE")

    # Parse API keys from environment if provided
    if os.environ.get("FAISS_AUTH_KEYS"):
        auth_keys = {}
        try:
            for key_pair in os.environ.get("FAISS_AUTH_KEYS").split(","):
